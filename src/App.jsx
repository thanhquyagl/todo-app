...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-300 flex flex-col pt-20 items-center gap-5">
      ...
      <div className="flex flex-col bg-white min-w-[600px] min-h-[432px] rounded py-4 px-5 shadow-md">
        <div className="flex">
          ...
          lists.map((item) => (
            <div className="group border-b py-3 px-2 mb-1 relative" key={item.id}>
              <label className="peer flex gap-2 items-center">
                <input className="peer size-3.5 appearance-none rounded-sm border border-slate-300 accent-pink-500 checked:appearance-auto" type="checkbox" name="1" />
                <span className="peer-checked:text-slate-400 peer-checked:line-through">
                  {item.todo}
                </span>
                <button className="bg-green-600 text-white px-2 py-1 text-sm rounded">edit</button>
              </label>
